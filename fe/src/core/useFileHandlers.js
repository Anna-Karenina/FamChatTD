  ...
  const  onSubmit = useCallback(
    (e, currentDialogId) => {
      ...
      if (state.files.length) {
        dispatch({ type: 'submit', currentDialogId })
      } else {
        window.alert("Нету файлов для загрузки")
      }
    },
    [state.files.length],
  )
  ...
  useEffect(() => {
    if (state.pending.length && state.next) {
      ...
      uploadApi
        ...
        .then(({data}) => {
          console.log(data)
          next.file._id = data.file._id
          next.file.oemname = data.file.filename
          const prev = next
          logUploadedFile(++countRef.current)
          const pending = state.pending.slice(1)
          dispatch({ type: 'file-uploaded', prev, pending, data })
        })
        .catch((error) => {
          console.error(error)
